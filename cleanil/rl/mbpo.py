...
from cleanil.dynamics.ensemble_dynamics import EnsembleDynamics, train_ensemble
...
@dataclass
class MBPOConfig(BaseTrainerConfig):
    # rollout args
    ...
    model_train_samples: int = 100000
    model_train_batch_size: int = 256
    model_train_steps: int = 500
    update_model_every: int = 250
...
def step_dynamics(
    ...
    obs["next"] = TensorDict(
        {"observation": next_obs, "reward": rwd, "terminated": terminated}, 
...
class Trainer(BaseTrainer):
    ...
    [1] When to Trust Your Model: Model-Based Policy Optimization, Janner et al, 2019 (https://arxiv.org/abs/1906.08253)
    """
    def __init__(
        self, 
        config: MBPOConfig,
        ...
        self.optimizers = {
            ...
            "dynamics": torch.optim.Adam(
                self.dynamics.parameters(), lr=config.lr_actor
            ),
        }
    ...
    def train_policy_epoch(self, global_step: int):
        ...
        fake_batch_size = 0
        if real_ratio > 0 and len(self.fake_buffer) >= batch_size:
            real_batch_size = int(batch_size * real_ratio)
            fake_batch_size = batch_size - real_batch_size

        policy_stats_epoch = []
        for _ in range(train_steps):
            batch = self.replay_buffer.sample(real_batch_size)
            if fake_batch_size > 0:
                fake_batch = self.fake_buffer.sample(fake_batch_size)
    ...
    def train_dynamics_epoch(self, global_step: int):
        ...
        model_train_batch_size = self.config.model_train_batch_size
        model_train_steps = self.config.model_train_steps
        model_train_samples = self.config.model_train_samples
        grad_clip = self.config.grad_clip

        # sample data without replacement
        sampler = copy.deepcopy(self.replay_buffer.sampler)
        self.replay_buffer.set_sampler(SamplerWithoutReplacement())
        data = self.replay_buffer.sample(batch_size=model_train_samples)
        self.replay_buffer.set_sampler(sampler)

        model_stats = train_ensemble(
            data,
            pred_rwd,
            self.dynamics,
    ...
    def sample_imagined_data(self, rollout_steps):
        rollout_batch_size = self.config.rollout_batch_size
        
        obs = self.replay_buffer.sample(rollout_batch_size).clone()
        obs = TensorDict(
            {
    ...
    def train(self):
        ...
        for t in pbar:
            ...
            if (t + 1) > update_after and (t - update_after + 1) % update_model_every == 0:
                model_stats_epoch = self.train_dynamics_epoch(t)
                ...
                if self.logger is not None:
                    ...
                    self.logger.log_scalar("dynamics/model_buffer_size", len(self.fake_buffer), t)
